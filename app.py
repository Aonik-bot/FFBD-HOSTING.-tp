...
def _otp_hash(code: str) -> str:
    return hmac.new(app.secret_key.encode('utf-8'), code.encode('utf-8'), hashlib.sha256).hexdigest()

def _valid_gmail(email: str) -> bool:
    return bool(re.fullmatch(r"[a-zA-Z0-9._%+\-]+@gmail\.com", (email or '').strip()))

def send_security_otp(to_email: str, purpose: str) -> bool:
    """Send a one-time verification code using SMTP. Never logs the OTP."""
    ...
    if not all([host, username, password, from_email]):
        app.logger.error('SMTP is not configured; refusing to send security OTP.')
        return False
    code = ''.join(secrets.choice('0123456789') for _ in range(6))
    msg = EmailMessage()
    msg['Subject'] = f'AONIK VIP security code — {purpose}'
    msg['From'] = from_email
    msg['To'] = to_email
    msg.set_content(
        f'Your AONIK VIP verification code is: {code}\n\n'
        f'This code expires in 10 minutes and can be used once. If you did not request it, ignore this email.'
    )
    try:
        ...
        session['otp_hash'] = _otp_hash(code)
        session['otp_expires_at'] = time.time() + OTP_TTL_SECONDS
        session['otp_attempts'] = 0
        return True
    except Exception:
        app.logger.exception('Security OTP delivery failed')
        return False

def verify_security_otp(code: str) -> bool:
    try:
        if time.time() > float(session.get('otp_expires_at', 0)):
            return False
        attempts = int(session.get('otp_attempts', 0))
        if attempts >= OTP_MAX_ATTEMPTS:
            return False
        session['otp_attempts'] = attempts + 1
        return hmac.compare_digest(_otp_hash(str(code).strip()), session.get('otp_hash', ''))
    except Exception:
        return False

def clear_security_otp():
...
app.secret_key = os.environ.get('SECRET_KEY', 'AONIK_VIP_SECRET_KEY_2026_SECURE')
...
if os.path.exists('/data'):
    ...
    persistent_db_path = '/data/aonik_vip.db'
    if not os.path.exists(persistent_db_path):
        try:
            os.makedirs('/data', exist_ok=True)
            original_db = os.path.join(BASE_DIR, 'database', 'aonik_vip.db')
    ...
    DB_PATH = os.path.join(BASE_DIR, 'database', 'aonik_vip.db')
...
def init_db():
    ...
    default_settings = {
        ...
        'site_title': 'AONIK VIP',
        'site_name': 'AONIK VIP',
        'vip_site_name': 'AONIK VIP',
        'self_ping_enabled': '1',
    ...
    if cursor.fetchone()[0] == 0:
        ...
        admin_pass = os.environ.get('ADMIN_PASSWORD', 'AONIK VIP')
        ...
        ''', ('AONIK VIP Super Administrator', 'admin', admin_email, admin_hash, 1000))
    ...
    primary_super_admins = ['oniko8768@gmail.com', 'onikislam3455@gmail.com', 'rrisankg11@gmail.com', 'admin']
        ...
        if enabled:
            ...
            for u in urls_to_ping:
                try:
                    clean_u = u.rstrip('/')
                    headers = {
                        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36 (AONIK VIP Self-Ping Bot)'
                    }
...
def is_user_super_admin(user):
    ...
    primary_super_admins = ['onikislam3457@gmail.com', 'risankg11@gmail.com', 'onikislam3456@gmail.com', 'onikislam3455@gmail.com']
...
def inject_global_vars():
    ...
    site_name = get_setting('site_name', 'AONIK VIP')
    if not site_name or not str(site_name).strip():
        site_name = 'AONIK VIP'
    else:
        site_name = str(site_name).strip()

    vip_site_name = get_setting('vip_site_name', 'AONIK VIP')
...
def health_check():
    return jsonify({'status': 'ok', 'app': 'AONIK VIP', 'time': datetime.utcnow().isoformat()})
...
DEFAULT_MAIN_PY = '''"""
AONIK VIP - Python Server Starter
...
def main():
    print(f"[{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [INFO] Starting Python application on AONIK VIP...")
    ...
    while True:
        ...
        print(f"[{now_str}] [INFO] AONIK VIP Heartbeat #{count} - Service running smoothly.")
        time.sleep(10)
...
def fetch_pypi_package_info(package_name):
    ...
    try:
        req = urllib.request.Request(url, headers={'User-Agent': 'AONIK VIP-Hosting-V2'})
...
def start_server_process(server_id):
    ...
    log_file.write(f"\n--- [AONIK VIP ENGINE] Server starting at {timestamp} ---\n")
    log_file.flush()
...
def signin():
    ...
    # Second step: verify OTP for the administrator.
    if request.method == 'POST' and request.form.get('otp_step') == '1':
        pending = session.get('pending_login') or {}
        code = request.form.get('otp', '').strip()
        if not pending:
            flash('Login verification expired. Please sign in again.', 'danger')
            return render_template('signin.html')
        if not verify_security_otp(code):
            flash('Invalid or expired verification code.', 'danger')
            return render_template('signin.html', otp_step=True, email=pending.get('email', ''))
    ...
    if request.method == 'POST':
        ...
        if not email or not password:
            flash('Please provide both your registered Gmail address and password.', 'danger')
            return render_template('signin.html', email=email)
        if not _valid_gmail(email):
            flash('Only a valid Gmail address (@gmail.com) can sign in.', 'danger')
            return render_template('signin.html', email=email)

        db = get_db()
        user = db.execute("SELECT * FROM users WHERE LOWER(email) = LOWER(?)", (email,)).fetchone()
        if not user:
            flash('Invalid Gmail address or password.', 'danger')
            return render_template('signin.html', email=email)
        ...
        # Admin login always requires a second factor sent to the configured admin Gmail.
        admin_email = os.environ.get('ADMIN_EMAIL', 'onikislam3455@gmail.com').strip().lower()
        is_admin = is_user_admin(user)
        if is_admin:
            if not admin_email or email != admin_email or not _valid_gmail(admin_email):
                flash('Administrator login is restricted to the configured admin Gmail address.', 'danger')
                return render_template('signin.html', email=email)
            session.clear()
            session['pending_login'] = {
                'user_id': user['id'],
                'email': email,
                'next_url': request.args.get('next') if request.args.get('next', '').startswith('/') else None,
            }
            if not send_security_otp(admin_email, 'administrator login'):
                session.pop('pending_login', None)
                clear_security_otp()
                flash('Could not send the administrator verification code. Configure SMTP first.', 'danger')
                return render_template('signin.html', email=email)
            flash('A verification code was sent to the administrator Gmail.', 'info')
            return render_template('signin.html', otp_step=True, email=email)

        # Normal users authenticate with their verified Gmail + password.
        session.clear()
        session['user_id'] = user['id']
        session['username'] = user['username']
        session['role'] = user['role']
    ...
    return render_template('signin.html', otp_step=False)

# 2.5 FORGOT PASSWORD API
# Password recovery is intentionally disabled. This prevents anyone from using
# the old CAPTCHA/email flow to replace an administrator password.
@app.route('/api/forgot-password/<path:unused>', methods=['GET', 'POST'])
def forgot_password_disabled(unused):
    return jsonify({'success': False, 'message': 'Password recovery is disabled. Contact the configured administrator.'}), 403
...
def signup():
    ...
    if request.method == 'POST' and request.form.get('verify_signup') == '1':
        pending = session.get('pending_signup') or {}
        code = request.form.get('otp', '').strip()
        if not pending:
            flash('Registration verification expired. Please start again.', 'danger')
            return render_template('signup.html')
        if not verify_security_otp(code):
            flash('Invalid or expired Gmail verification code.', 'danger')
            return render_template('signup.html', otp_step=True, email=pending.get('email', ''), full_name=pending.get('full_name',''), username=pending.get('username',''))
        email = pending['email']
        db = get_db()
        if db.execute("SELECT id FROM users WHERE LOWER(email) = LOWER(?)", (email,)).fetchone():
            clear_security_otp(); session.pop('pending_signup', None)
            flash('An account with this Gmail address already exists. Please sign in.', 'danger')
            return redirect(url_for('signin'))
        if db.execute("SELECT id FROM users WHERE LOWER(username) = LOWER(?)", (pending['username'],)).fetchone():
            clear_security_otp(); session.pop('pending_signup', None)
        ...
        cursor = db.cursor()
        cursor.execute('''INSERT INTO users (full_name, username, email, password_hash, coins, role, status)
                          VALUES (?, ?, ?, ?, ?, 'user', 'active')''',
                       (pending['full_name'], pending['username'], email, pw_hash, starting_coins))
        new_user_id = cursor.lastrowid
        vip_brand = get_setting('vip_site_name', 'AONIK VIP')
        if starting_coins > 0:
            cursor.execute('''INSERT INTO coin_transactions (user_id, amount, balance_after, description, transaction_type)
                              VALUES (?, ?, ?, 'Signup Welcome Bonus', 'credit')''', (new_user_id, starting_coins, starting_coins))
            cursor.execute('''INSERT INTO notifications (user_id, title, message) VALUES (?, ?, ?)''',
                           (new_user_id, '🎁 Welcome Bonus Coins', f"Welcome {pending['full_name']}! You received {starting_coins} free signup bonus coins."))
        else:
            cursor.execute('''INSERT INTO notifications (user_id, title, message) VALUES (?, ?, ?)''',
                           (new_user_id, f'Welcome to {vip_brand}', f"Welcome {pending['full_name']}! Your account has been registered successfully."))
        db.commit()
        clear_security_otp(); session.pop('pending_signup', None)
        session.clear()
        ...
        session['role'] = 'user'
        session.permanent = True
        flash('Account created successfully. Your Gmail address was verified.', 'success')
        return redirect(url_for('dashboard'))
    ...
    if request.method == 'POST':
        ...
        if not _valid_gmail(email):
            flash('Registration requires a valid Gmail address ending in @gmail.com.', 'danger')
            return render_template('signup.html', full_name=full_name, username=username, email=email)
        if len(username) < 3:
        ...
        if db.execute("SELECT id FROM users WHERE LOWER(email) = LOWER(?)", (email,)).fetchone():
            flash('An account with this Gmail address already exists. Please sign in.', 'danger')
            return render_template('signup.html', email=email, full_name=full_name, username=username)
        ...
        session['pending_signup'] = {'full_name': full_name, 'username': username, 'email': email, 'password_hash': safe_generate_password_hash(password)}
        if not send_security_otp(email, 'new account registration'):
            session.pop('pending_signup', None); clear_security_otp()
            flash('Could not send the Gmail verification code. Configure SMTP first.', 'danger')
            return render_template('signup.html', email=email, full_name=full_name, username=username)
        flash('A verification code was sent to your Gmail. Enter it below to finish registration.', 'info')
        return render_template('signup.html', otp_step=True, email=email, full_name=full_name, username=username)

    return render_template('signup.html', otp_step=False)
...
def admin_settings():
    ...
    if request.method == 'POST':
        ...
        if action == 'upload_logo':
            ...
            site_name = request.form.get('site_name', 'AONIK VIP').strip() or 'AONIK VIP'
            vip_site_name = request.form.get('vip_site_name', 'AONIK VIP').strip() or 'AONIK VIP'
...
if __name__ == '__main__':
    PORT = 3000
    print("==================================================")
    print("  AONIK VIP — PYTHON HOSTING PLATFORM       ")
